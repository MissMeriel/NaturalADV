...
def main(args):
    ...
    # results_dir = f"./{args.resultsparentdir}/dm2clear-SIZE{PERT_SIZE[0]}-BLUR{blur}-iters{iterations}-{args.ssimweight}ssim-{args.predweight}pred-{SD_IMG}-{rid}/"
    results_dir = f"./{args.resultsparentdir}/dm2clear-{SD_IMG}-LPIPS-{args.ssimweight}ssim-{args.predweight}pred-SIZE{PERT_SIZE[0]}-BLUR{blur}-iters{iterations}-{rid}/"
    ...
    initial_ssim_value = torch.mean(ssim(perturbation_sd_start, perturbation_orig, window_size=3))
    print("INITIAL SSIM:", initial_ssim_value)
    ...
    # print("ALL NEGATIVE? ", all([i < 0. for i in preds_pert_orig]))
    ...
    for it in range(iterations):
        ...
        ssim_out = -args.ssimweight * lpips_loss_fn.forward(imgs_pert_sd, imgs_pert_orig) - args.predweight * mse_loss(dave2(imgs_pert_sd), dave2(imgs_pert_orig))
        # ssim_out = -args.ssimweight * lpips_loss_fn.forward(perturbation_sd_start, perturbation) - args.predweight * mse_loss(dave2(imgs_pert_sd), dave2(imgs_pert_orig))

        ssim_out.backward()

        optimizer.step()
        print(f"ITER {it:03d} loss={ssim_out.item():.4f} ssim_loss={torch.mean(ssim_loss(perturbation_sd_start, perturbation, window_size=3)):.3f} mean grad={torch.mean(perturbation.grad).item()}")
        perturbation = torch.clamp(perturbation + torch.sign(perturbation.grad) / 100, 0., 1.)
        ssim_value = torch.mean(ssim(perturbation_sd_start, perturbation, window_size=3))
        print("CURRENT SSIM:", ssim_value)
    ...
    final_ssim_value = torch.mean(ssim(perturbation_sd_start, perturbation, window_size=3))
