...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-topo11.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
        ...
        elif default_scenario == "industrial" and default_spawnpoint == "straight1" and direction == "right":
            cube = ProceduralCube(name='cube_platform',
...
def setup_beamng(setup_args, vehicle_model='hopper',
    ...
    integral = 0.0
    prev_error = 0.0
    model = torch.load(f"{Path(__file__).parent}/../models/{model_name}", map_location=torch.device('cuda')).eval()
    setup_logging()
...
def run_scenario_to_collect_sequence(vehicle, bng, model, spawn, cuton=40, device=torch.device('cuda')):
    ...
    if not os.path.isdir(writedir):
        os.mkdir(writedir)
        print(f"Saving results to {writedir}")
...
def main():
    ...
    steps_per_sec = 15
    samples = 50
    newdir = "dissertation-TABLE1TOPO11-{}".format(''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
