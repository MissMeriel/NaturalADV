...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-topo1.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
...
def setup_beamng(setup_args, vehicle_model='hopper',
    ...
    # beamng = BeamNGpy('localhost', 64756, home=f'F:/BeamNG.research.v1.7.0.1', user=f'F:/BeamNG.researchINSTANCE2')
    # beamng = BeamNGpy('localhost', 65356, home=f'F:/BeamNG.research.v1.7.0.1', user=f'F:/BeamNG.researchINSTANCE3')
    beamng = BeamNGpy('localhost', 65400, home=f'F:/BeamNG.research.v1.7.0.1', user=f'F:/BeamNG.researchINSTANCE4')
...
# uses contour detection
@ignore_warnings
def get_qr_corners_from_colorseg_image_old(image):
    image = np.array(image)
    cv2.imshow('colorseg', image)
    cv2.waitKey(1)
    # hsv mask image
...
def superdeepbillboard(model, sequence, direction, steering_vector, bb_size=5, iterations=400, noise_level=25,
                       dist_to_bb=None, last_billboard=None, input_divers=True, loss_fxn='inv23', rs_overlap=None):
    ...
    # constraint = min(0.5, abs(1 - dist_to_bb / 32.0)*10) #math.atan(dist_to_bb / 22.0) # 0.2 # 0.1667
    print(f"{constraint=}")
...
def run_scenario(vehicle, bng, model, spawn, direction, run_number=0,
                                        ...
                                        bb_size=5, iterations=400, noise_level=25, dist_to_bb_cuton=37, resultsdir="images", input_divers=False, test_run_count=10):
...
def run_scenario_superdeepbillboard(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=0, run_number=0,
                                    ...
                                    input_divers=True, loss_fxn='inv23', test_run_count=10):
    ...
    save_image(torch.from_numpy(pert_billboards[-1]).permute(2, 0, 1) / 255.0, "{}/sdbb_pert_billboard_torchsaveimg.png".format(resultsdir))
    plt.close('all')
    pert_trajs, Ys = [], []
    ...
    while damage <= 0:
        ...
        if bbox_img is not None and kph > 29 and round(dist_to_bb,0) <= dist_to_bb_cuton: # and (steps % 2 < 1):
            ...
            pert_billboard, y, MAE = superdeepbillboard(model, sequence2, direction, copy.deepcopy(steering_vector2),
                                                   bb_size=bb_size, iterations=iterations, noise_level=noise_level,
                                                   dist_to_bb=dist_to_bb, input_divers=input_divers, loss_fxn=loss_fxn, rs_overlap=rs_overlap)
            ...
            cv2.imwrite(f"F:/Adv4Meee/im-adv-{len(pert_billboards):04d}.jpg", image[:, :, ::-1])
            cv2.imwrite(f"F:/Adv4Meee/im-orig-{len(pert_billboards):04d}.jpg", np.array(origimage)[:, :, ::-1])
            cv2.imwrite(f"F:/Adv4Meee/pert_billboard-{len(pert_billboards):04d}.jpg", np.array(pert_billboard)[:, :, ::-1])
            ...
            with open("F:/Adv4Meee/data.csv", "a") as f:
...
def main():
    ...
    samples = 10
    test_run_count = 10
    newdir = "dissertation-topo1left-smoothgrad-SSIMdiv10-negSSIM-blurpoolkernel3stride1-{}testruns-{}".format(test_run_count, ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
                                    ...
                                    if technique == "dbb":
                                        ...
                                        results = run_scenario(vehicle, bng, model, spawn, direction, collect_sequence_results=unperturbed_results,
                                                                                  bb_size=bbsize, iterations=its, noise_level=nl, dist_to_bb_cuton=cuton,
                                                                                    resultsdir=new_results_dir, test_run_count=test_run_count)
                                        ...
                                        results = run_scenario(vehicle, bng, model, spawn, direction,
                                                               ...
                                                               dist_to_bb_cuton=28,
                                                               resultsdir=new_results_dir, input_divers=False, test_run_count=test_run_count)
                                        ...
                                        results = run_scenario(vehicle, bng, model, spawn, direction,
                                                               ...
                                                               dist_to_bb_cuton=cuton, input_divers=input_div, resultsdir=new_results_dir, test_run_count=test_run_count)
                                    else:
