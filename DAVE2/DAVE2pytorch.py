...
class DAVE2v3(nn.Module):
    def __init__(self, input_shape=(100, 100)):
        ...
        self.features = nn.Sequential(self.conv1, self.pool1, self.conv2, self.pool2, self.conv3, self.pool3)
...
class DAVE2v4(nn.Module):
    def __init__(self, input_shape=(100, 100)):
        super().__init__()
        self.input_shape = input_shape

        self.conv1 = nn.Conv2d(3, 8, 3, stride=3)
        self.pool1 = nn.MaxPool2d(kernel_size=(2,2), padding=1)
        # torch.nn.MaxPool2d(kernel_size, stride=None, padding=0, dilation=1, return_indices=False, ceil_mode=False)
        self.conv2 = nn.Conv2d(8, 16, 3, stride=3)
        self.pool2 = nn.MaxPool2d(kernel_size=(2, 2), padding=1)
        self.conv3 = nn.Conv2d(16, 32, 2, stride=2)
        self.pool3 = nn.MaxPool2d(kernel_size=(2, 2), padding=1)
        self.conv4 = nn.Conv2d(32, 64, 2, stride=2)
        self.pool4 = nn.MaxPool2d(kernel_size=(2, 2), padding=1)

        size = np.product(nn.Sequential(self.conv1, self.pool1, self.conv2, self.pool2, self.conv3, self.pool3, self.conv4, self.pool4)(
            torch.zeros(1, 3, *self.input_shape)).shape)
    ...
    def forward(self, x):
        ...
        x = F.relu(x)
        x = self.pool4(x)
        encoding = x.flatten(1)
        x = self.lin1(encoding)
        x = F.relu(x)
        x = self.dropout1(x)
        x = self.lin2(x)
        x = F.relu(x)
...
class DAVE2smoothgrad(nn.Module):
    def __init__(self, input_shape=(100, 100)):
        super().__init__()
        self.input_shape = input_shape

        # self.features = nn.Sequential(nn.Conv2d(3, 16, 3, stride=3), nn.ReLU(True), nn.MaxPool2d(kernel_size=(2, 2), padding=1),
        #                               nn.Conv2d(16, 32, 3, stride=3), nn.ReLU(True), nn.MaxPool2d(kernel_size=(2, 2), padding=1),
        #                               nn.Conv2d(32, 64, 3, stride=3), nn.ReLU(True), nn.MaxPool2d(kernel_size=(2, 2), padding=1)
        #                               )

        self.features = nn.Sequential(nn.Conv2d(3, 24, 5, stride=2), nn.ReLU(True),
                                      nn.Conv2d(24, 36, 5, stride=2), nn.ReLU(True),
                                      nn.Conv2d(36, 48, 5, stride=2), nn.ReLU(True),
                                      nn.Conv2d(48, 64, 3, stride=1), nn.ReLU(True),
                                      nn.Conv2d(64, 64, 3, stride=1)
                                      )

        size = np.prod(self.features(torch.zeros(1, 3, *self.input_shape)).shape)

        self.regression = nn.Sequential(nn.Linear(in_features=size, out_features=500, bias=True), nn.ReLU(True), nn.Dropout(p=0.5),
                                        nn.Linear(in_features=500, out_features=100, bias=True), nn.ReLU(True), nn.Dropout(p=0.25),
                                        nn.Linear(in_features=100, out_features=20, bias=True), nn.ReLU(True),
...
class DAVE2v4smoothgrad(nn.Module):
    def __init__(self, input_shape=(100, 100)):
        super().__init__()
        self.input_shape = input_shape
...
class DAVE2extrasmoothgrad(nn.Module):
    def __init__(self, input_shape=(100, 100)):
        super().__init__()
        self.input_shape = input_shape
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 32, 5, stride=2), nn.LeakyReLU(), nn.BatchNorm2d(32),
                                nn.Conv2d(32, 32, 4, stride=2), nn.LeakyReLU(), nn.BatchNorm2d(32),
                                nn.Conv2d(32, 32, 4, stride=2), nn.LeakyReLU(), nn.BatchNorm2d(32),
                                nn.Conv2d(32, 32, 4, stride=1), nn.LeakyReLU(), nn.BatchNorm2d(32),
        )
        size = np.prod(self.features(torch.zeros(1, 3, *self.input_shape)).shape)
        print(f"feature vec {size=}")
        self.regression = nn.Sequential(
            nn.Dropout(0.5), nn.Linear(size, 512), nn.LeakyReLU(), nn.Linear(512, 1)
        )
