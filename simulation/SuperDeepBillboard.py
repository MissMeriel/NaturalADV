...
class SuperDeepBillboard():
    ...
    def __init__(self, model, seqpath, direction):
        ...
        self.natural_img = Image.open("F:/naturaladversarial/clear_billboard_50x50.jpg")
    ...
    def perturb_images(self, img_arr, img_patches, model, steering_vector,
        ...
        self.natural_img = transform(self.natural_img).unsqueeze(0).to(device)
        self.natural_img = self.natural_img / 255
        self.natural_img = self.natural_img.float()
        ...
        for i in range(iterations):
            ...
            if self.direction == "left" and loss_fxn == "MDirE":
                ...
                loss = (decay_factors * (y.flatten() - steering_vector)).mean()
            ssim_loss_calc = ssim_loss(self.natural_img, perturbation) / 10
            # print(f"Norms: {torch.norm(loss, 2)=:.5f} {torch.norm(ssim_loss_calc, 2)=:.5f}")
            # loss = loss / torch.norm(loss, 2) + ssim_loss_calc / torch.norm(ssim_loss_calc, 2)
            loss_combo = loss - ssim_loss_calc
            print(
                f"[iteration {i:5d}/{iterations}] loss_combo={loss_combo.item():2.5f} loss={loss.item():2.5f} ssim_loss={ssim_loss_calc.item():2.5f} max(angle)={y.max().item():2.5f} min(angle)={y.min().item():2.5f} mean(angle)={y.mean().item():2.5f} median(angle)={torch.median(y).item():2.5f}"
            )
            loss_combo.backward()
            ...
            # print(f"{perturbation.grad=}\n{torch.sign(perturbation.grad)=}\n{(perturbation - torch.sign(perturbation.grad))=}")
            # perturbation = torch.clamp(
            ...
            # smoothed_gradients = kornia.filters.guided_blur(guidance, 0.5 * perturbation.grad / torch.linalg.norm(perturbation.grad), 3, 0.1)
            # smoothed_gradients = kornia.filters.median_blur(0.5 * perturbation.grad / torch.linalg.norm(perturbation.grad), (5, 5))
            smoothed_gradients = kornia.filters.blur_pool2d(0.5 * perturbation.grad / torch.linalg.norm(perturbation.grad), 3, stride=1)
            # NOTE: only use smoothed gradients on large billboards, otherwise billboard will all be same color due to kernel size
            # print(f"{smoothed_gradients.shape=}")
            smoothed_gradients = transform_resize(smoothed_gradients)
            perturbation = torch.clamp(perturbation - smoothed_gradients, 0, 1)
            model.zero_grad()
            ssim_loss.zero_grad()
        print(
            f"[iteration {i:5d}/{iterations}] loss={loss_combo.item():2.5f} max(angle)={y.max().item():2.5f} min(angle)={y.min().item():2.5f} mean(angle)={y.mean().item():2.5f} median(angle)={torch.median(y).item():2.5f}"
        )
        y = model(imgs)
        y = y.cpu()
